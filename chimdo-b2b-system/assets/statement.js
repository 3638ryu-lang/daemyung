(function () {
  ...
  var distributorId = params.get('distributorId');
  var from = params.get('from');
  var to = params.get('to');
  var token = sessionStorage.getItem('chimdo_admin_token');

  if (!distributorId || !from || !to) {
  ...
  window.chimdoApi.call('getDistributorStatement', { token: token, distributorId: distributorId, from: from, to: to })
    .then(function (res) {
          ...
          '<tr>' +
            (idx === 0
              ? '<td rowspan="' + lineCount + '">' + formatDateOnly(o.Timestamp) +
                '<br><span style="color:#6b7280;font-size:0.76rem;">' + escapeHtml(o.OrderID) + '</span>' +
                (o.ResoldTo ? '<br><span style="color:#6b7280;font-size:0.76rem;">→ ' + escapeHtml(o.ResoldTo) + '</span>' : '') +
                '</td>'
              : '') +
